...
plt.rcParams.update({
    ...
    # The line below caused the error and is now removed.
    # Matplotlib will now use its default, reliable serif font.
    ''font.size": 14,
    ''axes.labelsize": 16,
    ''axes.titlesize": 18,
    ''xtick.labelsize": 12,
...
# 1. Crisp / Boolean definition
mu_crisp = np.where(h_range_full >= 1.8, 1, 0)
ax1.plot(h_range_full, mu_crisp, label='Crisp/Boolean', color='dodgerblue', linewidth=2.5)

# 2. Non-monotonic definition
h_points = [1.5, 1.6, 1.75, 1.85, 2.0]
mu_points = [0, 0.2, 0.8, 0.55, 1.0]
h_smooth = np.linspace(min(h_points), max(h_points), 200)
spline = make_interp_spline(h_points, mu_points, k=3)
mu_non_monotonic = np.clip(spline(h_smooth), 0, 1)
ax1.plot(h_smooth, mu_non_monotonic, label='Non-monotonic', color='crimson', linewidth=2.5, linestyle='--')
...
# Plotting the plausible functions
...
h_steps = [1.4, 1.6, 1.8, 1.9, 2.0, 2.1]
mu_values = [0, 0.3, 0.7, 1.0, 1.0, 1.0]
ax2.step(h_steps, mu_values, where='post', label='Step-wise', color='saddlebrown', linewidth=2.5, linestyle='-.')

# Add the thick ''Consensus" lines on top
...
plt.savefig('fuzzy_tall_definitions_final_v2.pdf', bbox_inches='tight', dpi=300)
plt.show()