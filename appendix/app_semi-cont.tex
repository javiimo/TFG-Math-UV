...
\begin{notation}{Multiplicative Generators}
    Parallel to additive generators, continuous Archimedean t-norms can also be represented using multiplicative generators. The key differences are:
    \begin{itemize}
    ...
    \end{itemize}
    The relationship between both types of generators is given by $\theta(x) = e^{-t(x)}$ and $t(x) = -\log(\theta(x))$.
    \end{notation}
...
\subsection{Representation of Continuous Archimedean T-norms}
A fundamental result, often attributed to Ling (1965) based on earlier work for topological semigroups by Mostert and Shields (1957), states that every continuous Archimedean t-norm possesses an additive generator \cite[Thm.~5.1]{Klement2000}. Specifically:
\textit{A function $T:^2 \to$ is a continuous Archimedean t-norm if and only if there exists a continuous, strictly decreasing function $t: \to [0, \infty]$ with $t(1)=0$, such that $T(x,y) = t^{(-1)}(\min(t(0), t(x)+t(y)))$. This generator $t$ is unique up to a positive multiplicative constant.}

This theorem is crucial because it guarantees that the entire class of continuous Archimedean t-norms can be characterized and constructed using these generator functions. Consequently, through the equivalence, they can also be represented by multiplicative generators.

\subsection{Constructing Families of T-norms from Generators}
Generators provide a powerful method for constructing parametric families of t-norms. The general approach involves:
\begin{enumerate}
    \item Starting with a known (prototypical) generator function. For example:
    \begin{itemize}
    ...
    \item Applying the construction formula (Equation~\eqref{eq:additive_generator_t_norm} or \eqref{eq:multiplicative_generator_t_norm}) with $t_\lambda(x)$ (or its corresponding $\theta_\lambda(x)$) to obtain a parametric family of t-norms $T_\lambda(x,y)$.
\end{enumerate}
...
\subsection{Examples of T-norm Families}
Below are some prominent families of continuous Archimedean t-norms, along with their definitions and key generator properties. These are based on the comprehensive Appendix A in \cite{Klement2000}.

\textbf{1. Schweizer-Sklar T-norms} ($T_\lambda^{SS}$) for $\lambda \in [-\infty, \infty]$ \cite[A.6]{Klement2000}
\begin{itemize}
...
\textbf{2. Hamacher T-norms} ($T_\lambda^H$) for $\lambda \in [0, \infty]$ \cite[A.4]{Klement2000}
\begin{itemize}
...
\textbf{3. Frank T-norms} ($T_\lambda^F$) for $\lambda \in [0, \infty]$ \cite[A.3]{Klement2000}
\begin{itemize}
...
\textbf{4. Yager T-norms} ($T_\lambda^Y$) for $\lambda \in [0, \infty]$ \cite[A.8]{Klement2000}
\begin{itemize}
...
\textbf{5. Dombi T-norms} ($T_\lambda^D$) for $\lambda \in [0, \infty]$ \cite[A.2]{Klement2000}
\begin{itemize}
...
\textbf{6. Sugeno-Weber T-norms} ($T_\lambda^{SW}$) for $\lambda \in [-1, \infty]$ \cite[A.7]{Klement2000}
\begin{itemize}
...
\textbf{7. Aczél-Alsina T-norms} ($T_\lambda^{AA}$) for $\lambda \in [0, \infty]$ \cite[A.1]{Klement2000}
\begin{itemize}
...
This overview demonstrates the utility of generators in unifying the understanding and construction of various t-norm families, clearly delineating their fundamental characteristics through the properties of these generating functions. clearly delineating their fundamental characteristics through the properties of these generating functions.