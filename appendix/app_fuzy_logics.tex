...
logical calculi admitting exact investigation}\cite[p.1]{Hajek1998}. A formal logic provides a precise framework for reasoning and is composed by a language (syntax), a meaning (semantics) and a proof system. Examples of formal logics are:
...
\paragraph{Syntax} defines the formal language: its vocabulary of basic symbols (used to define operations, predicates and constants) and formation rules for constructing well-formed formulas (WFFs)\footnote{This can be understood as grammar. It is just to exclude formulas that are incorrect. Some examples that do not make sense and would not be considered WFF are: "$))P\Rightarrow \neg\Rightarrow $" or "$P \land \Leftarrow \lor Q$".\cite[Sec.~2.3.3]{Agler2013SymbolicLogic}} (e.g., $(P \rightarrow Q)$ in PL \cite[Sec.~2.3.3]{Agler2013SymbolicLogic}). Figure~\ref{fig:fuzzy_syntax_diagram} summarises the jump from propositional to predicate syntax in the fuzzy setting, where $\otimes$ and $\to$ are a $t$-norm and its residuum.
...
\begin{figure}[!ht]
    ...
    \begin{tikzpicture}[node distance=1cm and 2.5cm]
        % title
        \node[title] (title_syntax) {Syntax: Propositional \textrightarrow{} Predicate Logic};

        % propositional syntax block
        \node[block, below=0.5cm of title_syntax, xshift=-1cm] (prop_syntax) {
            \textbf{Fuzzy Propositional Syntax}
            \begin{itemize}
                \setlength\itemsep{0em}
                \item Propositional variables ($p, q, \dots$)
                \item Truth constants ($0,1$)
                \item Connectives:
                  $\otimes$ (t\textendash{}norm conj.),
                  $\rightarrow$ (residuum),
                  $\neg$ (negation)
        ...
        % predicate syntax block
        \node[block_ext, right=of prop_syntax] (pred_syntax) {
            \textbf{Fuzzy Predicate Syntax (extends propositional)}

            \textcolor{pastelred!70!black}{\textit{Adds object structure:}}
            \begin{itemize}
                \setlength\itemsep{0em}
                \item Object variables ($x,y,\dots$), constants ($c$)
                \item Function symbols ($f, g, \dots$)
                \item Predicate symbols ($P, R, \dots$)
                \item Quantifiers ($\forall$, $\exists$)
                \item Terms and atomic formulas ($P(t_1,\dots,t_n)$)
    ...
    \end{tikzpicture}}
    \caption{Syntax elements for fuzzy propositional logic and their extension to fuzzy predicate logic.}
    \label{fig:fuzzy_syntax_diagram}
\end{figure}



\paragraph{Semantics} Truth is evaluated in \emph{models}. For classical PL the two-element Boolean algebra $\mathbf 2={0,1}$ and truth tables suffice. Fuzzy PL typically uses the unit interval $[0,1]$ equipped with a left-continuous $t$-norm $T$ and its residuum $\to_T$. Figure~\ref{fig:fuzzy_semantics_diagram} shows how the same truth-value algebra is reused when moving to fuzzy predicate logic, where a model additionally interprets symbols over a domain $U$ and quantifiers ($\oldforall, \exists$) by infima and suprema.
...
\begin{figure}[!ht]
    ...
    \begin{tikzpicture}[node distance=1cm and 2.5cm]
        % title
        \node[title] (title_sem) {Semantics: Propositional \textrightarrow{} Predicate Logic};

        % propositional semantics block
        \node[block, below=0.5cm of title_sem, xshift=-1cm] (prop_sem) {
            \textbf{Fuzzy Propositional Semantics}
            \begin{itemize}
                \setlength\itemsep{0em}
                \item Truth-value algebra $\mathbf{L}=\langle[0,1],\min,\max,T,\rightarrow_T,0,1\rangle$
                \item Evaluation $e$: variables $\to$ $[0,1]$
                \item Truth functions for $\otimes,\rightarrow,\neg$ given by $T,\rightarrow_T,1{-}x$
            \end{itemize}
        };

        % predicate semantics block
        \node[block_ext, right=of prop_sem] (pred_sem) {
            \textbf{Fuzzy Predicate Semantics (extends propositional)}

            \textcolor{pastelred!70!black}{\textit{Adds structure interpretation:}}
            \begin{itemize}
                \setlength\itemsep{0em}
                \item Domain of discourse $U$
                \item Interpretation $\mathcal{M}$ maps
                  \begin{itemize}
                      \setlength\itemsep{0em}
                      \item Constants $\to U$
                      \item $n$-ary functions $\to U^{n} \to U$
                      \item $n$-ary predicates $\to U^{n} \to [0,1]$
                  \end{itemize}
                \item Variable assignment $v: \text{ObjVars} \to U$
                \item Truth of atomic $P(t_1,\dots,t_n)$ via $\mathcal{M}(P)(v(t_1),\dots)$
                \item Connectives as in propositional case
                \item Quantifiers: 
                
                $\displaystyle \|
                    \forall x\,\varphi\|_{v}^{\mathcal{M}} 
                      = \inf_{u\in U}\|\varphi\|_{v[x\mapsto u]}^{\mathcal{M}}$


                  $\displaystyle \|\exists x\,\varphi\|_{v}^{\mathcal{M}} 
                      = \sup_{u\in U}\|\varphi\|_{v[x\mapsto u]}^{\mathcal{M}}$
            \end{itemize}
        };

        \path [line] (prop_sem.east) -- (pred_sem.west) node [midway, above, font=\footnotesize] {Extends with};
    \end{tikzpicture}}
    \caption{Semantic ingredients for fuzzy propositional logic and their extension to fuzzy predicate logic.}
    \label{fig:fuzzy_semantics_diagram}
\end{figure}

\paragraph{Proof System (or Deductive System)} A calculus supplies axioms and inference rules (e.g. Modus Ponens).  A \emph{proof} is a finite sequence each of whose lines is either an axiom or obtained from previous lines by a rule.  We write $\Gamma\vdash\varphi$ if $\varphi$ has a proof from premises $\Gamma$.  A theorem is a formula provable from the empty set.

\paragraph{Soundness and completeness.}  The fundamental goal of a logical system is to align its syntactic derivations with semantic truth. This is represented by two properties of a formal logic:
\begin{itemize}
\item \emph{Soundness}: $\Gamma\vdash\varphi\Rightarrow\Gamma\models\varphi$ (ensures that only valid formulas are provable).

\item \emph{Completeness}: $\Gamma\models\varphi\Rightarrow\Gamma\vdash\varphi$ (ensures that all valid formulas are provable).
\end{itemize}
When both hold, syntactic provability and semantic validity coincide. Semantic truth is achievable through syntactic manipulation.\cite[Thm.~1.2.11]{Hajek1998}


\section{Algebraic Logic}\label{app:alg_log}
Algebraic logic, pioneered by Tarski, studies logics through the equational properties of suitable \emph{truth‑value algebras}.  For classical PL these are Boolean algebras and for most fuzzy logics, \emph{residuated lattices}.  The bridge is built in two directions.

\paragraph{Syntactic side:}   For propositional logics, we can construct a canonical algebraic structure called the \textbf{Lindenbaum-Tarski algebra} by defining $\varphi\equiv\psi$ iff $\vdash(\varphi\leftrightarrow\psi)$. This represents that two formulas, $\phi$ and $\psi$, are considered equivalent (and belong to the same class, denoted $[\phi]$) if they are provably interchangeable within the logic. Since $\equiv$ is a congruence, the quotient of the term algebra forms an algebra $\mathbf{LT}(\mathcal L)$ on equivalence classes $[\varphi]$, where the logical connectives naturally induce operations on these equivalence classes (for example, $[\phi] \bar{\wedge} [\psi] = [\phi \wedge \psi]$).  A formula is provable exactly when its class is the top element $1$.

\paragraph{Semantic side (algebraic models):}  Fix a class $\mathcal K$ of algebras sharing the language of the logic.  A valuation is then a homomorphism from the term algebra to some $A\in\mathcal K$.  A formula $\varphi$ is a \emph{$\mathcal K$-tautology} if $v(\varphi)=1$ for every such valuation.  Soundness and completeness with respect to $\mathcal K$ mean

$$
  \text{provability}\;\Leftrightarrow\;\mathcal K\text{-tautology},
$$

which places $\mathbf{LT}(\mathcal L)$ inside the variety generated by $\mathcal K$.

Logics for which the bidirectional translation between proofs and algebraic equations, so that it is possible to fully represent logical inference using algebraic reasoning, are called algebraizable.\emph{algebraizable}\cite[Def.~2.10]{BlokPigozzi1989}.


\begin{example}
    Hájek's Basic Logic (BL) is sound and complete for the class of linearly ordered BL-algebras\cite[Thm.~2.3.15]{Hajek1998}.  It is \emph{not} complete for the single standard Łukasiewicz algebra on $[0,1]$ because the formula $\lnot\lnot x\leftrightarrow x$ holds there but is unprovable in BL, which shows that the entire variety is needed, since a particular algebra might satisfy more equations than the whole class does.
\end{example}


% \subsection{Residuated lattices}  
% A left-continuous $t$-norm $T$ possesses a \emph{residuum}
% \[
%   a\to_T b \;=\; \sup\{c \mid T(a,c)\le b\},
% \]
% and the algebra
% $\langle[0,1],\min,\max,T,\to_T,0,1\rangle$ is a \emph{commutative integral residuated lattice} (CIRL).

% \paragraph{Why this works.}
% Left-continuity of $T$ guarantees that the set
% \(\{\,c \mid T(a,c)\le b\}\) is closed and therefore that the supremum
% exists in \([0,1]\).  Moreover,
% \[
%    T(a,c)\le b \;\text{iff}\; c\le a\to_T b,
% \]
% so the pair $(T,\to_T)$ satisfies the defining \emph{residuation law}.
% Because the underlying lattice order on $[0,1]$ is total, we obtain a
% \emph{standard $T$-chain}.  In substructural and fuzzy logics this
% residuated structure algebraically models conjunction (\(\otimes\))
% and implication (\(\to\)), with the residuation law corresponding to
% modus ponens.  Esteva and Godo showed that the logic \textsf{MTL} is
% sound and complete for the whole class of such $T$-chains,
% demonstrating that left-continuity alone suffices to recover full
% implication-conjunction semantics \cite{GodoMonoidal}.  Hence every
% left-continuous $t$-norm embeds naturally into the residuated-lattice
% framework used throughout mathematical fuzzy logic.\\

% Adding just two equational schemata to this base yields
% \begin{center}
% \begin{tabular}{@{}lll@{}}
% \textbf{Logic} & \textbf{Extra law} & \textbf{Algebraic variety} \\ 
% MTL & prelinearity $(x\to y)\lor(y\to x)$ & MTL-algebras \\
% BL  & divisibility $x\land y = x\otimes(x\to y)$ & BL-algebras \\
% $\L$ (Łukasiewicz) & involution $\lnot\lnot x=x$ & MV-algebras \\
% $\mathbf G$ (Gödel) & idempotence $x\otimes x=x$ & Gödel algebras \\
% $\prod$ (Product) & cancellativity & Product algebras \\
% \end{tabular}
% \end{center}
% All sit inside the same residuated-lattice umbrella; completeness results differ (MTL is strongly complete for all l.c.\ $t$-norm chains\cite{JeneiMontagna2001}, BL is standard-complete for continuous $t$-norms\cite{CignoliEstevaGodo2000}, etc.).
