...
\begin{figure}[!ht]
    ...
    \begin{tikzpicture}[node distance=1cm and 2.5cm]
        ...
        \node[block_ext, right=of prop_syntax] (pred_syntax) {
            ...
            \begin{itemize}
                \setlength\itemsep{0em}
                \item \textcolor{pastelred!70!black}{\textit{All Propositional Syntax Elements}}
                \item \textbf{Plus:}
                \item Object Variables ($x, y$)
                \item Object Constants ($c, d$)
                \item Predicate Symbols ($P(-), R(-,-)$)
                \item Function Symbols ($f(-), g(-,-)$) (optional)
                \item Quantifiers ($\forall, \exists$)
                \item Terms (variables, constants, functions on terms)
    ...
    \end{tikzpicture}}
    \caption{Diagram summarizing the extension of syntax from fuzzy propositional logic to fuzzy predicate logic. Based on the ideas from \cite[Ch.~2,5]{Hajek1998}.}
