...
\signal{Aquí me falta introducir esto como que otra forma de entender los fuzzy numbers es que son un caso especial de fuzzy intervals. Y también motivar para qué me sirven los fuzzy numbers en MCDM: modelizar atributos, MISO, fuzzy linear programming, ANFIS y esas cosas.}
...
It's important to see that if $T(x,y) = \min(x,y)$, then $T(\xi, \eta) \ge \alpha$ implies $\xi \ge \alpha$ and $\eta \ge \alpha$. In this case, the union $\bigcup_{\min(\xi, \eta) \ge \alpha} f([A]^\xi, [B]^\eta)$ simplifies. Since $f([A]^\xi, [B]^\eta) \subseteq f([A]^\alpha, [B]^\alpha)$ for $\xi, \eta \ge \alpha$ (due to the nesting property of $\alpha$-cuts and assuming $f$ behaves well \signal{explicar eso de que se porte bien qué es} with set inclusion, e.g., for interval operations), the largest set in the union is $f([A]^\alpha, [B]^\alpha)$. Thus, Equation \eqref{eq:nguyen_generalized} reduces to:
\[
...
Esta sección igual la dejo igual no. Porque si acabo viendo lo de fuzzy TOPSIS por ejemplo, puede que necesite calcular distancias entre fuzzy numbers y me tocará definirlo. Si no, lo dejo fuera.
}
