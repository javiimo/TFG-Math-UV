...
The transition from classical to fuzzy sets necessitates extending the notion of truth from binary to graded values. When considering truth values in fuzzy logic, two fundamental questions arise. First, why use the unit interval [0,1]? And second, what does it mean for a proposition to have a specific truth degree like 0.7?

Regarding the first question, truth degrees in fuzzy logic are assumed to be linearly ordered, with 1 representing absolute truth and 0 representing absolute falsity. While finite truth value sets are possible, using the complete unit interval [0,1] provides a continuous scale that can accommodate any desired granularity. This aligns with real world scenarios like questionnaires where responses may be captured on various scales (e.g., 4 point or 11 point), all of which can be normalized to [0,1].

For the second question, the meaning of truth degrees must be considered differently for atomic versus compound propositions. For atomic propositions, truth degrees represent direct evaluations, such as responses to questions like "Do you like Haydn?" where answers like "absolutely yes" or "more or less" map to different points in [0,1]. Importantly, truth degrees across different propositions cannot be directly compared. That is, answering "absolutely yes" (1.0) to liking Haydn and "more or less" (0.7) to being old does not imply liking Haydn more than being old, just as classical yes/no answers would not be comparable across different questions.

For compound propositions, most fuzzy logics are truth functional, meaning the truth degree of a complex formula is determined by a function of the truth degrees of its components. For example, if disjunction is defined using the maximum function, then a truth degree of 0.7 for $\phi \lor \psi$ indicates that the maximum of the truth degrees of $\phi$ and $\psi$ is 0.7, constraining their possible values much like classical logic's truth tables constrain possible truth value combinations.\\ 
