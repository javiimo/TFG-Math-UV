...
The main idea behind this representation is leveraging the nilpotent elements in the diagonal. Start with all the diagonal being nilpotent (only possibility is $T_M$), and remove all the non-nilpotent intervals (where the t-norm is locally Archimedean) with one of the two possibilities in the continuous case: strict (isomorphic to $T_P$) or nilpotent (isomorphic to $T_L$).
